...
We can also adjust original DDLs to remove or change anything not supported by Liberatii. For this, just update the DDL1 and DDL2 columns of the corresponding ``dbt.migration_objects`` rows before running. DDL1 is executed on the ``schema`` stage, and DDL2 on the ``constraints`` stage.
