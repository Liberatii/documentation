...
The backward synchronization is kept for some time so it's possible to switch the application back to the original database in case of any trouble.

The synchronization is done in the ``sync`` stage.